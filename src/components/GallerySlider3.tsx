...
export default function GallerySlider3({
	className = '',
	galleryImgs,
    galleryImgsMsg,
    ...
    let currentMsg = galleryImgsMsg[index]
		...
		>
			...
			>
				...
				<div className={`w-full overflow-hidden ${galleryClass}`}>
                    ...
                    {currentMsg && (
                        <div className="absolute bottom-0 left-0 w-full text-white text-xs px-4 py-2 z-10 bg-gradient-to-r from-black/50 via-black/30 to-transparent rounded-b-lg">
                            {currentMsg}
                        </div>
                        )}
				</div>
